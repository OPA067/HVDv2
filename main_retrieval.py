...
def _run_on_single_gpu(model, batch_s_feat, batch_w_feat, batch_w_mask, batch_s_feat_list, batch_w_feat_list, batch_w_mask_list, batch_f_feat, batch_p_feat, split_batch=32):
    ...
    batch_s_feat_list = list(zip(*[torch.split(s_feat, split_batch) for s_feat in batch_s_feat_list]))
    batch_w_feat_list = list(zip(*[torch.split(w_feat, split_batch) for w_feat in batch_w_feat_list]))
    batch_w_mask_list = list(zip(*[torch.split(w_mask, split_batch) for w_mask in batch_w_mask_list]))
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    ids = []
    batch_s_feat, batch_w_feat, batch_w_mask = [], [], []
    batch_s_feat_list, batch_w_feat_list, batch_w_mask_list = [], [], []
    batch_f_feat, batch_p_feat = [], []
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_dataloader):
            ...
            batch_w_feat.append(w_feat)
            batch_w_mask.append(text_mask)
            batch_s_feat_list.append(s_feat_list)
            batch_w_feat_list.append(w_feat_list)
            batch_w_mask_list.append(text_mask_list)
            batch_f_feat.append(f_feat)
            batch_p_feat.append(p_feat)

        ids = allgather(torch.cat(ids, dim=0), args).squeeze()
        batch_s_feat = allgather(torch.cat(batch_s_feat, dim=0), args)
        batch_w_feat = allgather(torch.cat(batch_w_feat, dim=0), args)
        batch_w_mask = allgather(torch.cat(batch_w_mask, dim=0), args)
        batch_s_feat_list = list(zip(*batch_s_feat_list))
        batch_s_feat_list = [allgather(torch.cat(batch_s_feat, dim=0), args) for batch_s_feat in batch_s_feat_list]
        batch_w_feat_list = list(zip(*batch_w_feat_list))
        batch_w_feat_list = [allgather(torch.cat(batch_w_feat, dim=0), args) for batch_w_feat in batch_w_feat_list]
        batch_w_mask_list = list(zip(*batch_w_mask_list))
        batch_w_mask_list = [allgather(torch.cat(batch_w_mask, dim=0), args) for batch_w_mask in batch_w_mask_list]
        batch_f_feat = allgather(torch.cat(batch_f_feat, dim=0), args)
        batch_p_feat = allgather(torch.cat(batch_p_feat, dim=0), args)

    toc1 = time.time()

    with torch.no_grad():
        sim_matrix = _run_on_single_gpu(model, batch_s_feat, batch_w_feat, batch_w_mask, batch_s_feat_list, batch_w_feat_list, batch_w_mask_list, batch_f_feat, batch_p_feat, args.split_batch)
